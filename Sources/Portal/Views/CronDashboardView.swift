...
/// The Cron Activity surface as a scrolling stack of sections: the job
/// hierarchy, then activity metrics. Used on iOS (and as the pushed detail from
/// the cron list). On macOS the same sections are hosted as a free-form
/// draggable canvas — see `CronDashboardCanvas`.
///
/// The dataflow graph is deliberately *not* here. It moved to the **Graphs**
/// section (`GraphsView`), beside the wiki knowledge graph, where it gets a full
/// surface and a node inspector instead of a 320-point preview behind an Expand
/// button. The macOS canvas still offers it as an addable panel.
