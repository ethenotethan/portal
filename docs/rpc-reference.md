...
exposes for the checkout. `architecture.describe`'s `service` entry carries the
manifest's resolved `logs` sinks (`{id, kind: file|directory|launchd_stdout|launchd_stderr,
