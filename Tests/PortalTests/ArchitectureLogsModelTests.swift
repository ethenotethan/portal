...
internal struct ArchitectureLogsModelTests {
    ...
    internal func followEvents() async {
        ...
        reader.events.send(.architectureLog(ArchitectureLogEvent(service: "arch:portal", sink: "out", lines: ["b", "c"], cursor: "3", rotated: false, stopped: nil)))
        #expect(model.lines == ["a", "b", "c"])
        #expect(model.cursor == "3")
        reader.events.send(.architectureLog(ArchitectureLogEvent(service: "arch:portal", sink: "err", lines: ["ignored"], cursor: "9", rotated: false, stopped: nil)))
        reader.events.send(.architectureLog(ArchitectureLogEvent(service: "arch:other", sink: "out", lines: ["ignored"], cursor: "9", rotated: false, stopped: nil)))
        #expect(model.lines == ["a", "b", "c"], "other sinks and services are ignored")
        reader.events.send(.architectureLog(ArchitectureLogEvent(service: "arch:portal", sink: "out", lines: ["z"], cursor: "1", rotated: true, stopped: nil)))
        #expect(model.lines == ["z"])
        #expect(model.statusMessage == "Log rotated · tail restarted")
        reader.events.send(.architectureLog(ArchitectureLogEvent(service: "arch:portal", sink: "out", lines: [], cursor: "1", rotated: false, stopped: "idle-timeout")))
        #expect(!model.isFollowing)
