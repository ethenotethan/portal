...
# The observatory renderer, embedded in the app so Portal can present any
# service's architecture model natively (ArchitecturePanelPage splices the model
# into it). Generated from the site files below and gated by --check.
SITE_DIR = ROOT / "architecture/site"
OBSERVATORY_ASSETS_PATH = ROOT / "Sources/Portal/Models/ArchitectureObservatoryAssets.swift"
...
def swift_raw_literal(text: str) -> str:
    """A Swift raw multi-line string literal holding ``text`` verbatim: the pound
    count grows until neither the closing delimiter nor a raw interpolation
    marker can occur inside the content."""
    pounds = 1
    while f'"""{"#" * pounds}' in text or f"\\{'#' * pounds}(" in text:
        pounds += 1
    delimiter = "#" * pounds
    return f'{delimiter}"""\n{text}\n"""{delimiter}'


def observatory_assets_swift() -> str:
    """The site's index.html, app.js and styles.css as Swift constants."""
    parts = []
    for name, filename in (("indexHTML", "index.html"), ("appJS", "app.js"), ("stylesCSS", "styles.css")):
        text = (SITE_DIR / filename).read_text(encoding="utf-8").rstrip("\n")
        parts.append(f"    internal static let {name} = {swift_raw_literal(text)}\n")
    return (
        "// GENERATED by scripts/build_architecture.py from architecture/site — do not edit.\n"
        "// The Architecture Observatory renderer (index.html, app.js, styles.css), embedded so\n"
        "// Portal can present any service's architecture model natively: ArchitecturePanelPage\n"
        "// splices the model the gateway returns into this page. `make architecture` regenerates\n"
        "// it and `--check` fails when it drifts from the site files it mirrors. Excluded from\n"
        "// SwiftLint in .swiftlint.yml: the content is three web assets in raw string literals.\n"
        "import Foundation\n\n"
        "internal enum ArchitectureObservatoryAssets {\n"
        + "\n".join(parts)
        + "}\n"
    )
...
def expected_outputs() -> dict[Path, str]:
    ...
    return {
        ...
        OBSERVATORY_ASSETS_PATH: observatory_assets_swift(),
    }
