...
- **System flows** (`architecture/semantic/flows.json`): named paths whose every step is an edge the map draws (by history key), optionally starting from an observed trigger, connected step to step, with a note per step and an outcome. A flow whose edge has disappeared is `broken`, and the `flows-traceable` invariant fails the build until the flow is updated in the same change. Flows are listed as text beneath the invariants; **Trace on map** lights the path with the same highlighting a selection uses, numbering each step, and opens the flow in the side panel. Node inspectors list the flows they appear in.

`scripts/architecture_agent.py` writes both: `--constructs` batches constructs by kind into bounded packets (mechanical facts, the schema with its enums, fields already known mechanically, the current record, capped excerpts of the construct's own source), `--flows` sends the whole graph as `[from, relation, to]` triples plus the trigger table and asks for paths over them. Responses go through the compiler's validators; anything rejected is dropped with its reason printed and the previous record kept. Providers: an OpenAI-compatible endpoint (`OPENAI_API_KEY`, `OPENAI_BASE_URL`, `ARCHITECTURE_MODEL`) or the local `claude` CLI in print mode with structured output and no tools (`ARCHITECTURE_PROVIDER=claude-cli`, automatic when no key is set). `--stale-only` re-describes only constructs whose cited files changed. Enrichment never adds a node or an edge; nothing in placement, edges or invariants reads it.
