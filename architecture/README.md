...
## Extraction map: the projection made visible

The extractor is a function over the source tree. Each pass reads every Swift file and emits what its grammar recognises; whatever the grammar does not know does not exist on the map. That is a real trade-off: the grammar pulls the code toward shapes it can read (consistency), and it caps what the map can say (anything expressed another way is invisible to it and to every invariant built on it). The **Extraction map** view makes the projection itself visible instead of leaving the gap implicit.

The compiler derives `extraction` from the citations the rest of the model already carries. Every `(pass, file, line)` cited by a behavioral item, a map node, a trigger, a launch construction, a state machine, a store or an external-system usage is joined by body range to the declarations of the analysed tree (types, extensions and functions with a body, nested ones included). A declaration is **mapped** when a mechanical pass cites a line inside its body; a file is **touched** when any mechanical pass cites it. Citations written by a person (construct records, flows) are carried as the `semantic` class and never count as extraction. Because it is a pure function of the compiled model, the page can never disagree with the map.

The view's main diagram is the **provenance wiring**: the repository's file schema on the left (directories and files, closed until clicked), every construction the map drew on the right (grouped by kind), and one wire per extraction from the file and line a rule fired on to the entity it produced, coloured by rule family (store, behaviour, boundary, map wiring, trigger). Selecting a file lists what the extractor produced from it; selecting an entity lists every `file:line` and rule it came from and can open it on the system map. Beneath it, the same projection as area: every analysed file as a treemap cell sized by lines and shaded by the share of its declarations that were mapped; hatched cells were read and cited by nothing. Beneath it the same facts are reported **by construct** (declared vs mapped per kind), **by pass** (each rule of the grammar with the files it fired in and the lines it cited) and as the list of **untouched files**. Untouched is a fact about the extractor's grammar, not about the code: it is exactly the region the invariants do not cover, and the honest unit is a count of constructs, not a file percentage.
